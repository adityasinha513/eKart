...
/**
 * Rejects any request that doesn't carry the shared secret EkartGateway attaches to
 * every request it proxies. This is the only thing stopping a caller from reaching this
 * service directly (bypassing the Gateway's JWT check) in a local/dev environment where
 * network isolation between services isn't enforced yet.
 */
@Component
